...
* ``datastore.use_data_streams`` (default: ``false``): If ``true``, Rally writes metrics to the ``rally-metrics`` data stream instead of monthly ``rally-metrics-YYYY-MM`` indices. Rally creates a Rally-owned default ILM policy, the Rally-owned component templates, and the ``rally-metrics`` composable index template on demand, but does not create the data stream explicitly. The data stream is auto-created on the first write. Rally never overwrites an existing ``rally-metrics@custom`` component template.
